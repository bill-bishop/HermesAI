...
pub async fn spawn_noninteractive(cmd: Vec<String>, cwd: Option<String>) -> JobHandle {
    ...
    if let Some(out) = child.stdout.take() {
        let txc = tx.clone(); let seqc = latest_seq.clone();
        tokio::spawn(async move {
            let mut reader = BufReader::new(out).lines();
            while let Ok(Some(line)) = reader.next_line().await {
                let mut s = seqc.lock(); *s += 1;
                let _ = txc.send(StreamFrame{ t:"stdout".into(), seq:*s, d: format!("{line}\n") });
            }
        });
    }
    if let Some(err) = child.stderr.take() {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = reader.next_line().await {
                let mut s = seqc.lock(); *s += 1;
                let _ = txc.send(StreamFrame{ t:"stderr".into(), seq:*s, d: format!("{line}\n") });
            }
        });
    }
    let txe = tx.clone(); let seqe = latest_seq.clone(); let exite = exit_code.clone();
    tokio::spawn(async move {
    ...
    JobHandle { latest_seq, tx, exit_code }
}
