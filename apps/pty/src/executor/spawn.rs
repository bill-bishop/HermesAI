...
pub async fn spawn_noninteractive(cmd: Vec<String>, cwd: Option<String>) -> JobHandle {
    ...
    // helper: push a frame into backlog (ring) then broadcast
    let push_and_send = |frame: StreamFrame, tx: &tokio::sync::broadcast::Sender<StreamFrame>, backlog: &Arc<Mutex<VecDeque<StreamFrame>>>| {
